...
class VpcStack extends Stack {
    ...
    constructor(scope, id, props) {
        ...
        const { subnetCidr1, subnetCidr2 } = vpcAttr;

        // use an existing VPC if specified in options
        let vpc = {};
        let { vpcId } = vpcAttr;
        if (vpcId) {
            vpc = ec2.Vpc.fromLookup(this, 'vpc', { vpcId });
        }
        // or use the default VPC to keep things simple
        if (!vpcId) {
            vpc = ec2.Vpc.fromLookup(this, 'vpc', { isDefault: true });
            vpcId = vpc.vpcId;
        }
        const { vpcCidrBlock, availabilityZones } = vpc;
        this.vpcId = vpcId;
...
class AplicationStack extends Stack {
    ...
    constructor(scope, id, props) {
        ...
        // Certificate
        // Creating a certificate requires that DNS authorisation for ACM has already been configured in the zone
        let certificate = {};
